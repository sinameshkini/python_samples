...
#Config request
def config_req():
    data_link = "http://mehavira.ir/sajab/server.php?action=station&imei=9359374362&station_index=1"
    urlretrieve(data_link,"server_conf.txt")
    ser_conf = open("server_conf.txt","r")
    rcv = str(ser_conf.read())
    print(rcv)
    print(type(rcv))
    ...
    if len(rcv)>20:
        ...
        conf_str = "http://mehavira.ir/sajab/server.php?action=station&imei=9359374362&station_index=1&status=1&sampling_rate=%d&bias_value=%d&coefficent=%d&max_level=%d&hysteresis_value=%d&mobile_phone1=%s&mobile_phone2=%s&mobile_phone3=%s" %(sampling_rate,bias_value,coefficent,max_level,hysteresis_value,mobile_phone1,mobile_phone2,mobile_phone3)
...
#Send Data
def send_data():
    data_link = "http://mehavira.ir/sajab/server.php?action=save&station_index=1&ha=%d&hb=3&imei=9359374362" %(sensor1_read(sen1,temp1)[0])
    urlretrieve(data_link)
    save_data()
#End Send Data
#Sensor Read
...
def setasdefault():
    ...
    default_conf_str = "http://mehavira.ir/sajab/server.php?action=station&imei=9359374362&station_index=1&status=1&sampling_rate=%d&bias_value=%d&coefficent=%d&max_level=%d&hysteresis_value=%d&mobile_phone1=%s&mobile_phone2=%s&mobile_phone3=%s" %(default_sampling_rate,default_bias_value,default_coefficent,default_max_level,default_hysteresis_value,default_mobile_phone1,default_mobile_phone2,default_mobile_phone3)
...
Button(root,text="Setting",command=pass_check,width=10,font=(14)).place(x=630,y=230)
Button(root,text="Send Data",command=send_data,width=10,font=(14)).place(x=630,y=170)
Button(root,text="Config request",command=config_req,width=10,font=(14)).place(x=630,y=110)
